...
threshold = util.GetParamNumber("-threshold", 1.0e-3)--determinant threshold
beta = util.GetParamNumber("-beta", 10.0e0)--beta value, must be high
alpha =  util.GetParam("-alpha", 1.0e-10)--regularization parameter alpha, must be small
nu_geom = util.GetParamNumber("-nu_geom", 3.5e0)--geometrical penalty, must be high
...
updateParam= util.GetParam("-updateParam", 25)
ext_upper = util.GetParam("-ext_upper", 1.0)
ext_lower = util.GetParam("-ext_lower", 0.0)
nu_ext = util.GetParamNumber("-nu_ext", (ext_upper-ext_lower)/2)--extension factor
chi = util.GetParam("-chi", 1.0e-8)--penalization on the mean of the extension factor, increase for smoothness
limit_memory = util.GetParam("-limit_memory", 5)
control_tol = util.GetParamNumber("-control_tol", 1e-2)
geom_penalty_inc = util.GetParamNumber("-geom_penalty_inc", 1.5e0)
geom_constraints_tol = util.GetParamNumber("-geom_constraints_tol", 1e-2)
multiplier_scaling = util.GetParamNumber("-multiplier_scaling", 0.5e0)
stabType = util.GetParam("-stabType",0.005)
ext_overwrite=util.GetParam("-ext_overwrite",1000)
bOutput  = util.GetParamBool("-bOutput",true)--output VTK visualization files, all discretizations
bOutputPlot = util.GetParamBool("-bOutputPlot",true)--plots with gnuplot
bDoNothing = util.GetParamBool("-bDoNothing",true)--do nothing on flow outlet
bDeformationOutput = util.GetParamBool("-bDeformationOutput",true)--output deformation vector field
...
--gridName = util.GetParam("-grid", "./grids/box_2D_finer.ugx")
...
print("step size on boundary control:	"..stepSizeExt)
...
ReferenceVolume = VolumeDefect(w,0, "outer", "u1,u2",4,false,1,false)--this doesnot change
local vBarycenterDefect = {0,0,0}
volumeDefect = 0
LambdaVolume = 0
...
AdjointDeformation_ElemDisc = DeformationAdjointSystem("l1,l2","outer") 
...
--ExtensionEquation_Dirich:add(0,"k","outer")
...
function PrintStats() 
	if GetProfilerAvailable() == true then
		...
		--local scriptPath = "@/afs/math.uni-hamburg.de/users/oa/bay8214/ug4/apps/obstacle_optim/bfgs_surface_test.lua:"--compu5 Prof. Siebeneborn
		--local scriptPath = "@/afs/math.uni-hamburg.de/users/oa/bay8214/ug4/apps/obstacle_optim/bfgs_surface_test.lua:"--compu5 Jose
		local scriptPath = "@/home/alfon/ug4/apps/obstacle_optim/bfgs_surface_test.lua:"--cygwin
		--local scriptPath = "/zhome/academic/HLRS/xbo/xbopinzo/ug4/apps/obstacle_optim/bfgs_surface_test.lua:"--hawk
		local deformLine = 853 
		local navierLine = 890
		local adjointFlowLine=912
		local adjointDefLine=932
		local controlLine=946
		local extensionLine = 953 --search for: extensionLine to get line number
		stats = {
...
vBarycenterDefect = BarycenterDefect(w,"u1,u2","outer",4)
volumeDefect = VolumeDefect(w,ReferenceVolume, "outer", "u1,u2",4,false,1,false)
AdjointDeformation_ElemDisc:set_barycenter_defect(vBarycenterDefect[1],vBarycenterDefect[2],0)
AdjointDeformation_ElemDisc:set_volume_defect(volumeDefect)

--store norm of geometrical defects before update
vGeomNorm[step] = math.sqrt(volumeDefect*volumeDefect + vBarycenterDefect[1]*vBarycenterDefect[1] + vBarycenterDefect[2]*vBarycenterDefect[2])
	

if bDeformationOutput then
	...
	if bPrintSingleStep then
		vtkWriter:print(deformationOutputFile, w, 0, 0, true)
		vtkWriter:print_subsets("DeformationSurface",w,"obstacle_surface",0,0, false)
	else
		vtkWriter:print(deformationOutputFile, w, step, step, true)
		vtkWriter:print_subsets("DeformationSurface",w,"obstacle_surface",step,step, false)
		SaveVectorForConnectionViewer(w, "deformation.vec")
	end
end
...
if bExtensionEqOutput then
	...
	gnuplot.write_data("__Geometric_Data.txt", {vOptimStep,vVolume,vBarycenterX,vBarycenterY},false)
	gnuplot.write_data("__Lagrange_Data.txt", {vOptimStep,vLambdaVolume,vLambdaBarycenterX,vLambdaBarycenterY,vnu_geom},false)
...
vObjective[step] = drag + cont + thresh + vObjective_Volume[step] + vObjective_Barycenter[step] + ext
						--+ vObjective_LambdaVolume[step] + vObjective_LambdaBarycenter[step]
						
gnuplot.write_data("__Objective_Drag.txt", {vOptimStep,vObjective,vObjective_Drag,vObjective_Control,
											vObjective_Threshold,vObjective_Volume,vObjective_Extension,
											vObjective_Barycenter}, false)
...
gnuplot.write_data("__Control_Norm.txt",{vOptimStep, vControlNorm, vControlGradientNorm},false)	
	if bOutputPlot then 
		...
		if step > 0 then
			ctrlTol = math.abs(vControlNorm[step]-vControlNorm[step-1])/(vControlNorm[step])
		else 
			ctrlTol = 0
		end
		vControlConvergence[step] = ctrlTol
		if step > 0 and step%updateParam == 0 then	
		if ctrlTol < control_tol then
			local tol=math.sqrt(volumeDefect*volumeDefect + vBarycenterDefect[1]*vBarycenterDefect[1] + vBarycenterDefect[2]*vBarycenterDefect[2])
			print("Geometric defect norm is "..tol)
			if tol < geom_constraints_tol then
				print("NO UPDATE OF THE LAGRANGE MULTIPLIERS")
			else
			--calculate norms before update
			lambdaIncreaseNorm = math.sqrt(multiplier_scaling * nu_geom * volumeDefect* multiplier_scaling * nu_geom * volumeDefect
										 + multiplier_scaling * nu_geom * vBarycenterDefect[1]*multiplier_scaling * nu_geom * vBarycenterDefect[1]
										 + multiplier_scaling * nu_geom * vBarycenterDefect[2]*multiplier_scaling * nu_geom * vBarycenterDefect[2])
			...
			lambdaNorm = math.sqrt(LambdaVolume*LambdaVolume + LambdaBarycenter[1]*LambdaBarycenter[1] + LambdaBarycenter[2]*LambdaBarycenter[2])
			AdjointDeformation_ElemDisc:set_lambda_vol(LambdaVolume)
			AdjointDeformation_ElemDisc:set_lambda_barycenter(LambdaBarycenter[1],LambdaBarycenter[2],0)		
			--Reset BFGS methods
			--G_LBFGS:reset()
	...
	vLambdaNorm[step] = lambdaNorm
	vLambdaIncreaseNorm[step] = lambdaIncreaseNorm
	gnuplot.write_data("__Norms_Data.txt", {vOptimStep, vGeomNorm, vLambdaNorm, vLambdaIncreaseNorm, vControlConvergence},false)
